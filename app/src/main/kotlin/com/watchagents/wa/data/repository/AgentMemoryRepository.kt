...
internal data class AgentMemorySearchHit(
    val startLine: Int,
    val endLine: Int,
    val score: Double,
    val content: String,
...
internal class AgentMemoryStore(
    ...
    /** 语义检索：CJK 感知分词 + BM25 排序，返回按相关度排序的片段。 */
    fun search(
        query: String,
        maxHits: Int = AgentMemorySearchDefaults.MAX_HITS,
        maxChars: Int = AgentMemorySearchDefaults.MAX_CHARS,
    ): List<AgentMemorySearchHit> = synchronized(lock) {
        val snapshot = snapshotLocked()
        AgentMemorySemanticSearch.search(
...
internal object AgentMemorySearchDefaults {
    const val MAX_HITS = 6
...
internal object AgentMemoryRepository {
    ...
    fun search(
        ...
        ensureInitialized()
        return store.search(query, maxHits, maxChars)
    }
