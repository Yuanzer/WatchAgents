...
import com.watchagents.wa.data.db.ProviderWithModelsSeed
import com.watchagents.wa.data.db.toDomain
import com.watchagents.wa.data.db.toEntity
import com.watchagents.wa.data.db.toModelEntities
...
import com.watchagents.wa.data.model.withApiKey
import com.watchagents.wa.data.model.withModels
import com.watchagents.wa.data.model.withSortOrder
...
internal object ProviderRepository {
    ...
    suspend fun ensureBuiltInsMerged() {
        ...
        // 内置默认 Key 回填：仅在 DEFAULT_DEEPSEEK_API_KEY 非空时生效（当前默认为空 = 不预置 Key）
        val deepseek = providerById(BuiltinProviders.DEEPSEEK_ID)
        if (deepseek != null && deepseek.apiKey.isBlank() &&
            BuiltinProviders.DEFAULT_DEEPSEEK_API_KEY.isNotBlank()
        ) {
            replaceProvider(deepseek.withApiKey(BuiltinProviders.DEFAULT_DEEPSEEK_API_KEY))
        }
    }
    ...
    private fun dao() =
        WaDatabase.get(appContext()).providerDao()
