...
import com.watchagents.wa.agent.runtime.AgentRunCancelledException
...
import com.watchagents.wa.data.model.CustomBody
import com.watchagents.wa.data.model.CustomHeader
import com.watchagents.wa.data.model.OpenAiEndpointMode
import com.watchagents.wa.data.model.ModelReasoningCapabilities
...
internal object AgentModelClient {
    ...
    /**
     * 根据用户第一条消息为会话生成简短主题标题。
     * 轻量单次调用（不带工具、不进入会话记录）；失败或不可用时返回 null，由调用方回退到前缀标题。
     */
    fun generateConversationTitle(
        config: ModelConfig,
        firstUserMessage: String,
    ): String? {
        runCatching { config.validate() }.getOrNull() ?: return null
        val messages = JSONArray()
            .put(
                JSONObject()
                    .put("role", "system")
                    .put("content", TITLE_SYSTEM_PROMPT)
            )
            .put(
                JSONObject()
                    .put("role", "user")
                    .put("content", firstUserMessage.take(TITLE_INPUT_MAX_CHARS))
            )
    ...
    private fun String.cleanConversationTitle(): String? {
        var cleaned = trim()
        for (prefix in listOf("\"", "'", "「", "『", "《", "【", "(", "（")) {
            cleaned = cleaned.removePrefix(prefix).removeSuffix(prefix)
        }
        return cleaned
            .lineSequence()
            .firstOrNull()
    ...
    private const val TITLE_MAX_CHARS = 16
    private const val TITLE_INPUT_MAX_CHARS = 2_000
    private const val TITLE_SYSTEM_PROMPT =
        "你是会话命名助手。根据用户的第一条消息，为这段对话生成一个简短的中文主题标题：" +
            "概括这次对话的话题或目标，不超过 12 个字。直接输出标题本身，" +
            "不要引号、冒号、解释、编号或任何标点。示例：" +
            "“帮我看看军训安排” → 军训安排查询；" +
            "“分析一下为什么我的钉钉会被封号” → 钉钉封号原因分析；" +
            "“打开网易云音乐播放每日推荐” → 播放每日推荐。"
    ...
    data class ModelConfig(
        ...
        val browserTools: Boolean = false,
