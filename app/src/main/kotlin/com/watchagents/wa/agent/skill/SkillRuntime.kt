package com.watchagents.wa.agent.skill

import android.content.Context
import android.content.res.AssetManager
import com.watchagents.wa.data.db.WaDatabase
import com.watchagents.wa.data.db.SkillRegistryEntity
...
private class BuiltinSkillAssetStore(
    ...
    fun seedMissingBuiltins(registryStore: SkillRegistryStore) {
        ...
        listBuiltins().forEach { builtin ->
            ...
            val safeInstalled = isSafeBuiltinSkillInstallation(targetDir)
            // App 升级后内置 SKILL.md 更新：仅同步资产文件（SKILL.md 等），
            // 保留技能自己沉淀的数据目录（如 self-improving-agent/data）。
            val contentChanged = safeInstalled && !builtinContentMatches(builtin, targetDir)
            when {
                !safeInstalled -> installBuiltinInternal(builtin)
                contentChanged -> refreshBuiltinContent(builtin)
            }
            if (
                entry?.source == BUILTIN_SOURCE &&
                entry.installState == INSTALL_STATE_INSTALLED &&
                !contentChanged
            ) {
                return@forEach
            }
    ...
    /** 已安装目录中的 SKILL.md 与资产包是否一致（内容字节比较）。 */
    private fun builtinContentMatches(builtin: BuiltinSkillAsset, targetDir: File): Boolean =
        runCatching {
            val asset = context.assets.open("${builtin.assetPath}/SKILL.md").bufferedReader().use { it.readText() }
            val installed = File(targetDir, "SKILL.md").readText()
            asset == installed
        }.getOrDefault(true)

    /** 只把资产目录中的文件同步进已存在目录，不清除技能自身沉淀的数据。 */
    private fun refreshBuiltinContent(builtin: BuiltinSkillAsset) {
        copyAssetRecursively(context.assets, builtin.assetPath, File(skillsRoot, builtin.id))
    }
