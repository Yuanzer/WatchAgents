...
-keep,allowoptimization,allowobfuscation class com.watchagents.wa.ModuleMain {
    public <init>();
}
...
# ── Release 日志策略 ────────────────────────────────────────────────────────
# 仅删除 WA 自有代码中的 Android VERBOSE/DEBUG 调用；INFO/WARN/ERROR 必须保留，
# 第三方依赖的日志策略由依赖自身决定。
-maximumremovedandroidloglevel 3 class com.watchagents.wa.** { *; }

# XposedModule.log 不是 android.util.Log，R8 无法通过上面的规则识别。
# debug supplier 是纯观察 API；禁止在 supplier 内执行任何业务副作用。
-assumenosideeffects interface com.watchagents.wa.core.AgentLogger {
    public abstract void debug(kotlin.jvm.functions.Function0);
}
-assumenosideeffects class com.watchagents.wa.core.AndroidAgentLogger {
    public void debug(kotlin.jvm.functions.Function0);
}
-assumenosideeffects class com.watchagents.wa.core.ModuleLogger {
    public void debug(kotlin.jvm.functions.Function0);
}
